...
export default function CarsPage() {
  ...
  useEffect(() => {
    fetch("/api/cars")
      .then(res => res.json())
      .then(data => {
        setCars(Array.isArray(data) ? data : [] as any);
        setLoading(false);
      })
      .catch(() => {
  ...
  if (!Array.isArray(cars) || cars.length === 0) {
    return <div className="text-center text-gray-500 pt-24">
      {isClient ? (lang === 'AR' ? 'لا توجد سيارات.' : 'Aucune voiture trouvée.') : ''}
    </div>;
  }
    ...
    <main className="min-h-screen bg-gray-50">
      <section className="max-w-5xl mx-auto px-4 py-18">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-8 text-center">
          {isClient ? (lang === 'AR' ? 'جميع السيارات' : 'Toutes les voitures') : ''}
        </h1>
