...
export default function ManageCarsPage() {
  ...
  if (cars.length === 0) return <div className="text-center text-gray-500 pt-24">Aucune voiture trouvée.</div>;
          ...
          {cars.map((car: any) => (
            <div key={car._id} className="bg-white rounded-xl shadow-md p-6 flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="flex flex-col gap-2">
                <Link href={`/owner/manage-cars/${car._id}/edit`} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg text-sm text-center">Éditer</Link>
                <button onClick={() => handleDelete(car._id)} className="bg-red-100 hover:bg-red-200 text-red-700 font-bold py-2 px-4 rounded-lg text-sm">Supprimer</button>
                <Link href={`/cars/${car._id}`} className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded-lg text-sm text-center">Voir</Link>
              </div>
            </div>
          ))}
        </div>
