      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto flex flex-col-reverse md:flex-row items-center justify-between gap-10">
        ...
        <div className="w-full md:w-1/2 flex flex-col items-center md:items-start">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-center md:text-left mb-4 text-gray-800 leading-tight">
            <span className="block">{isClient ? t("buyCar") : ""}</span>
            <span className="block text-indigo-600">{isClient ? t("ofYourDreams") : ""}</span>
          </h1>
          <p className="text-gray-600 text-center md:text-left mb-8 max-w-md">
            {isClient ? t("findPerfectCar") : ""}
          </p>
          <form
          ...
          >
            <div className="space-y-5">
              {/* Champ de ville */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{isClient ? t("city") : ""}</label>
                <div className="relative">
                  ...
                  >
                    <option value="">{isClient ? t("selectCity") : ""}</option>
                    {cityList.map(city => (
              ...
              {/* Champ de marque */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{isClient ? t("brand") : ""}</label>
                <div className="relative">
                  <select
                    value={selectedBrand}
                    onChange={e => {
                  ...
                  >
                    <option value="">{isClient ? t("allBrands") : ""}</option>
              ...
              {/* Champ de modèle */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{isClient ? t("model") : ""}</label>
                <div className="relative">
                  <select
                    value={selectedModel}
                    ...
                    className="w-full pl-4 pr-4 py-3.5 text-gray-700 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 appearance-none"
                  >
                    <option value="">{isClient ? t("allModels") : ""}</option>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <div className="flex items-center">
              ...
              <span className="text-sm text-gray-600">{isClient ? t("noHiddenFees") : ""}</span>
            </div>
            <div className="flex items-center">
              <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center mr-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <span className="text-sm text-gray-600">{isClient ? t("secureTransaction") : ""}</span>
            </div>
          </div>
        </div>
